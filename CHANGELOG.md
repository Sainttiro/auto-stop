...
- **CRITICAL: Исправлено отображение открытых позиций в /stats_detailed**
  ...
  - Причина: Использовался `get_portfolio()` который возвращает `instrument_type` (тип), а не тикер
  - Решение: Переход на `get_positions()` (рекомендуемый метод API) + получение тикера через `get_instrument_by_figi()`
  - Результат: Теперь отображаются реальные тикеры (SBER, GAZP и т.д.)
  - Бонус: Валютные позиции (баланс счета) автоматически исключаются

### 🔄 Изменено
- Метод `_get_actual_positions()` теперь использует `get_positions()` вместо `get_portfolio()`
- Обработка `positions_response.securities` вместо `portfolio.positions`
- Прямой API запрос для получения тикера по FIGI
